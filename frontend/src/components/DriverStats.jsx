...
function DriverStats({ userId }) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const res = await api.get("/v1/bookings/driver/stats");

      const data = res?.data || {};

      setStats({
        assignedTrips: Number(data.assignedTrips || 0),
        completedTrips: Number(data.completedTrips || 0),
        vehicleStatus: data.vehicleStatus || "—",
      });
    } catch (error) {
      console.error("Error fetching driver stats:", error?.response?.data || error);
      setStats({
        assignedTrips: 0,
        completedTrips: 0,
        vehicleStatus: "—",
    ...
    return (
      <div className="driver-stats-grid">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="driver-stat-card skeleton"
            style={{ height: "120px" }}
          >
